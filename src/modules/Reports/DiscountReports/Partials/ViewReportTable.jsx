...
export const MoveSlider = styled.div`
  ...
  transition: all 0.5s;
`;
export const ViewAllDiscountReports = () => {
...
// =========  Filter Show =======
  ...
  const DateSearch = async (data) => {
    setLoading(true);
    await baseRequest
      .post(APIURLS.POST_DISCOUNT_DATE, data)
  ...
  const onFinish = (values) => {
    ...
    if(choiceFull === 'date'){
        ...
        const monthvalues = {
          ...
          console.log(monthvalues, "valuesSucess");
    }

  };
  console.log(choiceFull, "choiceFull");
  ...
  const TableColumns = [
    ...
    },
    {
      ...
      render: (value) => (
        <CustomTag title={value} bordered={"true"} color={"warning"} />
      ),
    },
    {
      title: "Discount %",
  ...
  return (
    <Fragment>
      <CustomPageTitle Heading={"Discount Reports"} />
      <br />
      <Form
        form={form}
        labelCol={{
                ...
                {show ? (
                  <CustomRow space={[24, 24]}>
                    ...
                    <Col span={24} md={24} lg={6} >
                      <Flex>
                        <ButtonStandard.Primary text={"Submit"}  loading={loading} htmlType="submit" />
                      </Flex>
                    </Col>
                  </CustomRow>
                ) : null}
